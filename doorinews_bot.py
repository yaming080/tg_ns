...
# ===== 2026-06-06 stability patch: no googletrans fallback / hard ending normalize / image recovery =====
...
def ensure_story_image(story: dict) -> dict:
    image_url = (story.get('image_url') or '').strip()
    link = (story.get('url') or '').strip()
    desc = (story.get('desc') or '').strip()

    need_meta_fetch = (not is_valid_image_url(image_url))
    if need_meta_fetch and link:
        try:
            article_desc, article_img = fetch_article_meta(link)
            if article_img and is_valid_image_url(article_img):
                story['image_url'] = article_img
            if is_weak_text(desc) and article_desc:
                story['desc'] = article_desc
        except Exception as e:
            log(f"[meta 이미지 복구 실패] {link} | {e}")

    return story


def normalize_korean_endings_hard(text: str) -> str:
    if not text:
        return ""
...
def cleanup_summary_before_send(text: str) -> str:
    text = text or ""
    text = _clean_summary_for_style_v3(text)
    text = fix_broken_inline_hashtags(text)
    text = remove_duplicate_inline_hashtags(text)
    text = finalize_summary_ending(text)
    text = normalize_korean_endings_hard(text)
    text = re.sub(
        r'\n\s*(#(?:[A-Za-z0-9가-힣_()]+)(?:\s+#(?:[A-Za-z0-9가-힣_()]+))*)\s*\n(?=\s*🌐|\s*출처|\s*$)',
        '\n',
        text
    )
    return text.strip()
...
def build_message(story: dict) -> str:
    ...
    summary_ko, dynamic_tags = inject_entity_hashtags(summary_ko, entities)
    summary = cleanup_summary_before_send(summary_ko)
...
def send_telegram_photo(token: str, channel: str, image_url: str, caption: str) -> bool:
    ...
    if not caption or not caption.strip():
        log("Empty caption -> skip")
        return False

    if not is_valid_image_url(image_url):
        log("No valid image_url -> text message로 전송")
        return send_telegram_message(token, channel, caption)
...
def main():
    ...
    for story in new_stories:
        story = ensure_story_image(story)
        msg = build_message(story)
        if not msg:
            log(f"[메시지생성실패 스킵] {story.get('title','')}")
            continue
    ...
    log(f"실행 요약 | 전체수집={len(collected)} | 필터통과={len(filtered)} | 최종전송={posted_count}")
